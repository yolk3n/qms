...
Процедура ПриПолученииНастроек(Настройки) Экспорт
	...
	Алгоритмы.ПриПодключенииККорреспонденту         = Истина;
	...
	Настройки.ИмяКонфигурацииПриемника                       = Новый Структура("БухгалтерияГосударственногоУчреждения");
...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	ОписаниеВарианта.ИмяФайлаНастроекДляПриемника          = НСтр("ru = 'Настройки обмена для БА2 - БГУ2'");
	ОписаниеВарианта.ИмяКонфигурацииКорреспондента         = ПустаяСсылка().Метаданные().Представление();
	ОписаниеВарианта.КраткаяИнформацияПоОбмену             = НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""Больничная аптека"", и ""Бухгалтерия государственного учреждения"", ред. 2.0.
	...
	ОписаниеВарианта.ПояснениеДляНастройкиПараметровУчета  = НСтр("ru = 'Перед выполнением обмена необходимо заполнить информацию счета учета по умолчанию в регистре сведений Счета учета операций.
		|Это необходимо для корректного проведения документов.'");
	
	ОписаниеВарианта.НаименованиеКонфигурацииКорреспондента = ПустаяСсылка().Метаданные().Представление();
...
//
// Параметры:
//  ВерсияКорреспондента - Строка - версия конфигурации корреспондента, например, "2.0.0.1".
//
Процедура ПриПодключенииККорреспонденту(ВерсияКорреспондента) Экспорт
	
	МинимальноПоддерживаемаяВерсияКорреспондента = "2.0.0.1";
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияКорреспондента, МинимальноПоддерживаемаяВерсияКорреспондента) < 0 Тогда
		ТекстОшибки = НСтр("ru = 'Настройка синхронизации данных поддерживается только с конфигурацией
			|""Бухгалтерия государственного учреждения"" версии %1 и выше.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, МинимальноПоддерживаемаяВерсияКорреспондента);
...
Процедура НастроитьИнтерактивнуюВыгрузку(Получатель, Параметры) Экспорт
	...
	СправочникиНеСинхронизируются = Получатель.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать;
	ДокументыНеСинхронизируются   = Получатель.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.НеВыгружать;
...
Функция ПредставлениеОтбораИнтерактивнойВыгрузки(Получатель, Параметры) Экспорт
	...
	Если СписокОрганизаций.Количество() > 0 Тогда
		...
		ПредставлениеОрганизаций = ОбщегоНазначенияБольничнаяАптекаКлиентСервер.СокращенноеПредставлениеКоллекцииЗначений(СписокОрганизаций);
		ОписаниеОтбораОрганизации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ОписаниеОтбораОрганизации, ПредставлениеОрганизаций);
