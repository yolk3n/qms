...
Процедура ЗагрузитьТребования(НачалоПериода = Неопределено, КонецПериода = Неопределено) Экспорт
	...
	ФайлXML = XML_GetDEPAWMHS(НачалоПериода,КонецПериода); 
...
Процедура ЗагрузитьРасходныеДокументы(НачалоПериода,КонецПериода) Экспорт
	...
	ФайлXML = XML_GetAPTRWMHS(НачалоПериода,КонецПериода);
