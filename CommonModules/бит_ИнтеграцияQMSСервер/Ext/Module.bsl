...
Процедура ЗаданиеОбмена(ДатаНачала = Неопределено, ДатаОкончания = Неопределено) Экспорт
	...
	ЗагрузитьТребования(ДатаНачала,ДатаОкончания);
	ЗагрузитьРасходныеДокументы(ДатаНачала,ДатаОкончания);
...
Процедура ЗагрузитьТребования(ДатаНачала = Неопределено, ДатаОкончания = Неопределено) Экспорт
	...
	Если Ответ <> Неопределено Тогда		
		...
		ДокументыQMS.Свернуть("DocType,DocNum,DocDate,DocOTD,DocSKL,DocOTDTO,DocSKLTO");
...
Процедура ЗаполнитьРасходныеДокументы(Шапка, ТаблицаНоменклатуры)
	...
	ШапкаДокумента = ЗаполнитьШапкуДокумента(Шапка, ТаблицаНоменклатуры);
	Если ШапкаДокумента.ИмяДокумента = Неопределено Тогда
		Возврат
	КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаНоменклатуры Цикл 
		...
		ДанныеСтроки.Вставить("Коэффициент",?(ДанныеСтроки.КоличествоВЕдиницахИзмерения = 0, 0,
								ДанныеСтроки.Количество / ДанныеСтроки.КоличествоВЕдиницахИзмерения));
		ДанныеСтроки.Вставить("СтатусУказанияСерий", 6);
		ДанныеСтроки.Вставить("СтатусУказанияПартий", 4);
		
		ДанныеСтроки.Вставить("СтатьяРасходов",?(Номенлатура1С.ЭтоЛекарственноеСредство,Справочники.бит_НастройкиИнтеграцииQMS.СтатьяРасходовЛС.Значение,
											Справочники.бит_НастройкиИнтеграцииQMS.СтатьяРасходовРМ.Значение)); 
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Строка,"ProductSerial") И ТипЗнч(Строка.ProductSerial) = Тип("Строка") Тогда
			ДанныеСтроки.Вставить("СерияНоменклатуры", Справочники.СерииНоменклатуры.НайтиПоРеквизиту("Номер",Строка.ProductSerial,,ДанныеСтроки.Номенклатура));
			Если Не ЗначениеЗаполнено(ДанныеСтроки.СерияНоменклатуры) И ЗначениеЗаполнено(ДанныеСтроки.Номенклатура) Тогда
				ДанныеСтроки.Вставить("СерияНоменклатуры", СоздатьСериюНоменклатуры(ДанныеСтроки.Номенклатура,Строка.ProductSerial,Строка.ValidityPeriod));
			КонецЕсли;
		Иначе
		...
		Если ИсточникФинансирования = Неопределено Тогда
			...
			ШапкаДокумента.Вставить("ИсточникФинансирования", ИсточникФинансирования); 
		КонецЕсли;
		ДанныеСтроки.Вставить("ИсточникФинансирования", ИсточникФинансирования);
		ПараметрыПоискПартии = ПолучитьСтруктуруПараметровПоискПартии(ШапкаДокумента, ДанныеСтроки);
		ДанныеСтроки.Вставить("Партия", ПодобратьПартию(ПараметрыПоискПартии));
...
Функция ЗаполнитьШапкуДокумента(Шапка, ТаблицаНоменклатуры)
	...
	ШапкаДокумента.Вставить("Отделение",  ?(ЗначениеЗаполнено(Шапка.DocOTDTO),ОбъектПоКоду(Шапка.DocOTDTO),ОбъектПоКоду(Шапка.DocOTD)));
	ШапкаДокумента.Вставить("ПодразделениеОрганизации", Справочники.бит_НастройкиИнтеграцииQMS.ПодразделениеАптека.Значение);
	...
	ШапкаДокумента.Вставить("СкладПолучатель", ?(ЗначениеЗаполнено(Шапка.DocSKLTO),ОбъектПоКоду(Шапка.DocSKLTO),Справочники.бит_НастройкиИнтеграцииQMS.СкладАптеки.Значение));
	...
	ШапкаДокумента.Вставить("СрокБрони",ШапкаДокумента.Дата + 7*86400);
	
	ТЗ = НоваяТаблицаProductTransferStructure();
	Для Каждого Элемент Из ТаблицаНоменклатуры Цикл
		 ЗаполнитьЗначенияСвойств(ТЗ.Добавить(),ТаблицаНоменклатуры);
	КонецЦикла;
	ШапкаДокумента.Вставить("Хэш",ХэшДанных(ТЗ)); 
...
Процедура ЗаписатьДокумент(ДанныеДокумента)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыПоиска,ДанныеДокумента.ШапкаДокумента);	
	...
	//Если ДокОбъект.Проведен Тогда
		ЗаписатьХэшСсылки(ДокОбъект.Ссылка, ДанныеДокумента.ШапкаДокумента.Хэш)
	//КонецЕсли;
...
Функция НайтиДокумент(ПараметрыПоиска)
	...
	НомерДокумента = ПараметрыПоиска.НомерQMS;
	ДатаДокумента = ПараметрыПоиска.Дата;
	Отделение = ПараметрыПоиска.Отделение;
	ХэшДокумента = ПараметрыПоиска.Хэш;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПустаяСсылка,"НомерQMS") И 
			ЗначениеЗаполнено(бит_ИнтеграцияQMSСерверПовтИсп.ДополнительноеСведениеХэш()) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|      И (ДополнительныеСведения.Свойство.Имя = ""Хэш_qMS"")
		|ГДЕ
...
Функция ПодобратьПартию(Параметры)
	...
	Запрос.УстановитьПараметр("КонецПериода", Параметры.ДатаДокумента);
	Запрос.УстановитьПараметр("Организация", Параметры.Организация);
	Запрос.УстановитьПараметр("Склад", Параметры.Склад);
	Запрос.УстановитьПараметр("Номенклатура", Параметры.Номенклатура);
	Запрос.УстановитьПараметр("СерияНоменклатуры", Параметры.СерияНоменклатуры);
...
Процедура ЗаписатьХэшСсылки(Ссылка, Хэш)
	...
	Свойство = бит_ИнтеграцияQMSСерверПовтИсп.ДополнительноеСведениеХэш();
...
Функция ДополнительноеСведениеПоИмени(Имя) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(ПВХ) Тогда
		...
		ПВХОбъект.Имя = Имя;
		//ПВХОбъект.НаборСвойств = Справочники.НаборыДополнительныхРеквизитовИСведений.;
...
Функция ПолучитьСтруктуруПараметровПоискПартии(Шапка,СтрокаНоменклатура)
	
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("Номенклатура", СтрокаНоменклатура.Номенклатура);
	СтруктураПараметров.Вставить("СерияНоменклатуры", СтрокаНоменклатура.СерияНоменклатуры);
	СтруктураПараметров.Вставить("ЕдиницаИзмерения", СтрокаНоменклатура.ЕдиницаИзмерения);
	СтруктураПараметров.Вставить("Количество", СтрокаНоменклатура.КоличествоВЕдиницахИзмерения);
	СтруктураПараметров.Вставить("Организация", Шапка.Организация);
	СтруктураПараметров.Вставить("Склад", Шапка.Склад);
	СтруктураПараметров.Вставить("ДатаДокумента", Шапка.Дата);
...
Функция НовыеПараметрыПоискаДокумента() 
	...
	ПараметрыПоиска.Вставить("ИмяДокумента");
	ПараметрыПоиска.Вставить("НомерQMS");
	ПараметрыПоиска.Вставить("Дата");
...
Функция НоваяСтруктураСообщенияЖурнала() Экспорт 
	...
	СтруктураСообщения.Вставить("Объект", Неопределено);
