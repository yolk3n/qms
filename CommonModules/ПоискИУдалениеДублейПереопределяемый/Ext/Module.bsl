...
Процедура ПриОпределенииОбъектовСПоискомДублей(Объекты) Экспорт
	...
	// БольничнаяАптека
	Если Константы.КонтролироватьУникальностьРабочегоНаименованияНоменклатурыИСерии.Получить() Тогда
		ИменаОбработчиков = "ПараметрыПоискаДублей" + Символы.ПС + "ПриПоискеДублей";
		Объекты.Вставить(Метаданные.Справочники.Номенклатура.ПолноеИмя()     , ИменаОбработчиков);
		Объекты.Вставить(Метаданные.Справочники.СерииНоменклатуры.ПолноеИмя(), ИменаОбработчиков);
	КонецЕсли;
