...
Процедура ПриСозданииФормыМонитора(Форма, ПараметрыСоздания) Экспорт
	...
	// БольничнаяАптека
	Если Форма.ИмяФормы = Метаданные.Обработки.МониторПортала1СИТС.Формы.Форма.ПолноеИмя() Тогда
		Элементы = Форма.Элементы;
		ПроверяемыеКартинки = Новый Массив;
		ПроверяемыеКартинки.Добавить(Элементы.Декорация36);
		ПроверяемыеКартинки.Добавить(Элементы.Декорация39);
		ПроверяемыеКартинки.Добавить(Элементы.Декорация42);
		ПроверяемыеКартинки.Добавить(Элементы.Декорация48);
		ПроверяемыеКартинки.Добавить(Элементы.Декорация45);
		Для Каждого ПроверяемаяКартинка Из ПроверяемыеКартинки Цикл
			Если ПроверяемаяКартинка.Картинка.Вид = ВидКартинки.Пустая Тогда
				// Картинка ДлительнаяОперация16 из БИП не используется, т.к. дублирует картинку БСП. Ошибка БИП.
				ПроверяемаяКартинка.Картинка = БиблиотекаКартинок.ДлительнаяОперация16;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
