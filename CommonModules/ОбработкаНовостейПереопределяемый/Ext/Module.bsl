...
Процедура ДополнительноОбработатьФормуПросмотраНовостейДляРабочегоСтолаПриСозданииНаСервере(
	...
	// БольничнаяАптека
	Если Форма.ИмяФормы = Метаданные.Справочники.Новости.Формы.ФормаПросмотраНовостейРабочийСтол.ПолноеИмя() Тогда
		Элементы = Форма.Элементы;
		ПроверяемыеКартинки = Новый Массив;
		ПроверяемыеКартинки.Добавить(Элементы.КартинкаВыполнениеФоновогоЗадания);
...
Процедура ЗначениеПредопределеннойКатегории(Категория, Значение) Экспорт
	...
	КонецЕсли;
	Если КатегорияКод = "ProductName" Тогда
