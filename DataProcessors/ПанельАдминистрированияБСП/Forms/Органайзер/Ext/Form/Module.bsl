...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВнешниеРодительскиеКонстанты = Новый ФиксированнаяСтруктура(ОбщегоНазначенияБольничнаяАптекаПовтИсп.ПолучитьРодительскиеКонстанты(СоставНабораКонстантФормы));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ВнешниеРодительскиеКонстанты.Свойство(Источник) Тогда
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если (РеквизитПутьКДанным = "НаборКонстант.ИспользоватьБизнесПроцессыИЗадачи" ИЛИ РеквизитПутьКДанным = "")
		...
		// БольничнаяАптека
		ДоступностьБизнесПроцессовИЗадач = ОбщегоНазначенияБольничнаяАптека.ВозможноИзменитьЗначениеКонстанты(
			"ИспользоватьБизнесПроцессыИЗадачи", НаборКонстант.ИспользоватьБизнесПроцессыИЗадачи);
		...
		Элементы.ИспользоватьПодчиненныеБизнесПроцессы.Доступность  = НаборКонстант.ИспользоватьБизнесПроцессыИЗадачи И ДоступностьБизнесПроцессовИЗадач;
...
Функция ТекстКомментарияИспользоватьБизнесПроцессы()
	...
	Если Константы.ИспользоватьБизнесПроцессыТребованийОтделений.Получить() Тогда
		
		ДополнениеККомментариюТребования = НСтр("ru = 'Согласование и исполнение требований в разделе ""Отделения""'");
		ДополнениеККомментарию = ДополнениеККомментарию + ?(Не ПустаяСтрока(ДополнениеККомментарию), ";", "") + Символы.ПС + Символы.Таб + "- " + ДополнениеККомментариюТребования;
		
	КонецЕсли;
	
	Если Константы.ИспользоватьБизнесПроцессыВнутреннихЗаказов.Получить() Тогда
		
		ДополнениеККомментариюЗаказы = НСтр("ru = 'Согласование и исполнение внутренних заказов в разделе ""Запасы""'");
		ДополнениеККомментарию = ДополнениеККомментарию + ?(Не ПустаяСтрока(ДополнениеККомментарию), ";", "") + Символы.ПС + Символы.Таб + "- " + ДополнениеККомментариюЗаказы;
		
	КонецЕсли;
	
	Если Константы.ИспользоватьБизнесПроцессыФормуляров.Получить() Тогда
		
		ДополнениеККомментариюФормуляры = НСтр("ru = 'Согласование и утверждение формулярных списков в разделе ""Предприятие""'");
		ДополнениеККомментарию = ДополнениеККомментарию + ?(Не ПустаяСтрока(ДополнениеККомментарию), ";", "") + Символы.ПС + Символы.Таб + "- " + ДополнениеККомментариюФормуляры;
