...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстанавливатьАктуальнуюВерсию = Истина;
	...
	Подразделение = Справочники.ПодразделенияОрганизаций.ПолучитьПодразделениеПоУмолчанию(Организация);
	ПодразделениеДоИзменения = Подразделение;
	
	ИсточникСведений = ИсточникСведений_МинЗдрав();
	ИсточникСведенийДоИзменения = ИсточникСведений;
	
	УстановитьТипКлассификатора();
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
		Текст = НСтр("ru = 'Реквизит ""Код OID"" организации не заполнено.'");
		ОбщегоНазначения.СообщитьПользователю(Текст, Организация, "КодOID",, Отказ);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Подразделение) Тогда
		ИдентификаторПодразделения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "КодOID");
		Если Не ЗначениеЗаполнено(ИдентификаторПодразделения) Тогда
			Текст = НСтр("ru = 'Реквизит ""Код OID"" подразделения не заполнено.'");
			ОбщегоНазначения.СообщитьПользователю(Текст, Подразделение, "КодOID",, Отказ);
		КонецЕсли;
	КонецЕсли;
	
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() Тогда
		...
		КонецЕсли;
	ИначеЕсли ИсточникСведений = ИсточникСведений_ФедеральныеРегистры() Тогда
...
Процедура ВерсияПриИзменении(Элемент)
	...
	Если ВерсияДоИзменения = Версия Тогда
		Возврат;
	КонецЕсли;
	ВерсияДоИзменения = Версия;
	...
	УстанавливатьАктуальнуюВерсию = Версия = Элементы.Версия.СписокВыбора[0].Значение;
	ТекущийНомерПорции = 1;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() И Не ЗначениеЗаполнено(Версия) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодразделениеПриИзменении(Элемент)
	...
	Если ПодразделениеДоИзменения = Подразделение Тогда
		Возврат;
	КонецЕсли;
	ПодразделениеДоИзменения = Подразделение;
	
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() 
			И (Не ЗначениеЗаполнено(Версия) Или Не ЗначениеЗаполнено(Организация)) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ИсточникСведенийПриИзменении(Элемент)
	
	Если ИсточникСведенийДоИзменения = ИсточникСведений Тогда
		Возврат;
	КонецЕсли;
	ИсточникСведенийДоИзменения = ИсточникСведений;
	
	УстановитьДоступныеВерсииКлассификатора();
...
////////////////////////////////////////////////////////////////////////////////
// Получение данных классификатора установленной версии
...
Функция ПолучитьДанныеКлассификатораВФоне()
	...
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() Тогда
		ФильтрыПолей = ОбменССервисомМинздраваРФ.ФильтрыПолейЗагрузкиДанныхКлассификатора_Отделения();
		ФильтрыПолей.ИдентификаторОрганизации   = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "КодOID");
		ФильтрыПолей.ИдентификаторПодразделения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "КодOID");
		
		ПараметрыМетода = ОбменССервисомМинздраваРФ.ПараметрыПолучения_ДанныеКлассификатора(ТипКлассификатора, Версия, ТекущийНомерПорции, РазмерПорции, "Наименование", ФильтрыПолей);
		
		ИмяПроцедуры = "ОбменССервисомМинздраваРФ.ПолучитьДанныеКлассификатора";
	ИначеЕсли ИсточникСведений = ИсточникСведений_ФедеральныеРегистры() Тогда
		ПараметрыМетода = ОбменССервисомФедеральныхРегистров.ПараметрыПолучения_ДанныеКлассификатора(ТипКлассификатора, Организация);
		ПараметрыМетода.Вставить("ИдентификаторПодразделения", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "КодOID"));
		
		ИмяПроцедуры = "ОбменССервисомФедеральныхРегистров.ПолучитьДанныеКлассификатора";
	Иначе
		ВызватьИсключение(НСтр("ru = 'Неизвестный источник сведений. Обратитесь к администратору.'"));
	КонецЕсли;
...
Функция ЗагрузитьДанныеКлассификатораВФоне(ИдентификаторыСтрок)
	...
	ИсточникСведенийМЗ = (ИсточникСведений = ИсточникСведений_МинЗдрав());
	ИсточникСведенийФР = (ИсточникСведений = ИсточникСведений_ФедеральныеРегистры());
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если ТекущиеДанные.ЗагрузкаДоступна Тогда
			Если ИсточникСведенийМЗ Тогда
				ЭлементДанных = ОбменССервисомМинздраваРФ.СтруктураДанныхКлассификатора_Отделения();
			ИначеЕсли ИсточникСведенийФР Тогда
				ЭлементДанных = ОбменССервисомФедеральныхРегистров.СтруктураДанныхКлассификатора_Отделения();
			Иначе
				ВызватьИсключение(НСтр("ru = 'Неизвестный источник сведений. Обратитесь к администратору.'"));
			КонецЕсли;
	...
	Если ИсточникСведенийМЗ Тогда
		ПараметрыМетода = ОбменССервисомМинздраваРФ.ПараметрыЗагрузки_ДанныеКлассификатора(ТипКлассификатора, ДанныеКлассификатора);
		ИмяПроцедуры = "ОбменССервисомМинздраваРФ.ЗагрузитьДанныеКлассификатора";
	ИначеЕсли ИсточникСведенийФР Тогда
		ПараметрыМетода = ОбменССервисомФедеральныхРегистров.ПараметрыЗагрузки_ДанныеКлассификатора(ТипКлассификатора, ДанныеКлассификатора);
...
Функция ОбработатьКэшированныеДанныеКлассификатора()
	...
	ИдентификаторОрганизации = ИсточникСведений + "_" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "КодOID");
	...
	Если ЗначениеЗаполнено(Подразделение) Тогда
		ИдентификаторПодразделения = ИсточникСведений + "_" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "КодOID");
	Иначе
		ИдентификаторПодразделения = ИсточникСведений + "_" + "ВсеПодразделения";
	КонецЕсли;
	ЗагруженныеДанныеПодразделения = ЗагруженныеДанныеОрганизации.Получить(ИдентификаторПодразделения);
	Если ЗагруженныеДанныеПодразделения = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	
	ВерсияКэша = ?(ИсточникСведений = ИсточникСведений_МинЗдрав(), Версия, ИсточникСведений);
	ЗагруженныеДанныеВерсии = ЗагруженныеДанныеПодразделения.Получить(ВерсияКэша);
	Если ЗагруженныеДанныеВерсии = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	
	ОбщееКоличествоДанных = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЗагруженныеДанныеВерсии, "ОбщееКоличествоДанных", 0);
	Если ОбщееКоличествоДанных = 0 Тогда
		Возврат Ложь; // Нет данных в базе сервиса.
	КонецЕсли;
	
	ДанныеКлассификатора = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЗагруженныеДанныеВерсии, "ДанныеКлассификатора");
	...
	Для Индекс = 0 По Граница Цикл
		...
		НайденныйЭлемент = ОбщегоНазначенияБольничнаяАптекаКлиентСервер.НайтиВМассивеСтруктур(ДанныеКлассификатора, Новый Структура("КлючСтроки", Индекс + Смещение));
	...
	ОтобразитьПорциюДанных(ЗагруженныеДанныеВерсии);
...
Процедура ОчиститьКэшированныеДанные()
	...
	Если ЗагруженныеДанныеВерсии = Неопределено Тогда
		Возврат;
	Иначе
		ЗагруженныеДанныеВерсии = Неопределено;
	КонецЕсли;
	
	ЗагруженныеДанныеПодразделения.Вставить(ВерсияКэша, ЗагруженныеДанныеВерсии);
	ЗагруженныеДанныеОрганизации.Вставить(ИдентификаторПодразделения, ЗагруженныеДанныеПодразделения);
...
Процедура ОбработатьНовыеДанныеКлассификатора(Результат, ЗаменятьСуществующиеДанные = Ложь)
	...
	ЗагруженныеДанныеОрганизации = ЗагруженныеДанные.Получить(ИдентификаторОрганизации);
	Если ЗагруженныеДанныеОрганизации = Неопределено Тогда
		ЗагруженныеДанныеОрганизации = Новый Соответствие;
	КонецЕсли;
	...
	ЗагруженныеДанныеПодразделения = ЗагруженныеДанныеОрганизации.Получить(ИдентификаторПодразделения);
	Если ЗагруженныеДанныеПодразделения = Неопределено Тогда
		ЗагруженныеДанныеПодразделения = Новый Соответствие;
	КонецЕсли;
	
	ВерсияКэша = ?(ИсточникСведений = ИсточникСведений_МинЗдрав(), Версия, ИсточникСведений);
	ЗагруженныеДанныеВерсии = ЗагруженныеДанныеПодразделения.Получить(ВерсияКэша);
	Если ЗагруженныеДанныеВерсии = Неопределено Тогда
		ЗагруженныеДанныеВерсии = Новый Структура("ОбщееКоличествоДанных, ДанныеКлассификатора");
		ЗагруженныеДанныеВерсии.ОбщееКоличествоДанных = Результат.ОбщееКоличествоДанных;
		ЗагруженныеДанныеВерсии.ДанныеКлассификатора  = ОбщегоНазначения.СкопироватьРекурсивно(Результат.ДанныеКлассификатора);
	Иначе
		ДанныеКлассификатора = ЗагруженныеДанныеВерсии.ДанныеКлассификатора;
		Для Каждого ЭлементДанных Из Результат.ДанныеКлассификатора Цикл
			НайденныйЭлемент = ОбщегоНазначенияБольничнаяАптекаКлиентСервер.НайтиВМассивеСтруктур(ДанныеКлассификатора, Новый Структура("КлючСтроки", ЭлементДанных.КлючСтроки));
			Если НайденныйЭлемент = Неопределено Тогда
				ДанныеКлассификатора.Добавить(ОбщегоНазначения.СкопироватьРекурсивно(ЭлементДанных));
...
Процедура ОтобразитьПорциюДанных(ДанныеВерсииКлассификатора)
	...
	Для Счетчик = НижнийПредел По ВерхнийПредел - 1 Цикл
		НайденныйЭлемент = ОбщегоНазначенияБольничнаяАптекаКлиентСервер.НайтиВМассивеСтруктур(ДанныеКлассификатора, Новый Структура("КлючСтроки", Счетчик));
		Если НайденныйЭлемент <> Неопределено Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ГруппаОтбораИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		Элемент.Отбор.Элементы, "Цвет", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	
	ГруппаОтбораИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ГруппаОтбораИли.Элементы, "НекорректныеДанные", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ГруппаОтбораИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ГруппаОтбораИли.Элементы, "ДанныеКПометкеУдаления", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИ,
		"Классификатор.ЗагрузкаДоступна", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИ,
		"Классификатор.ДатаУпразднения", ВидСравненияКомпоновкиДанных.Заполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИ,
		"Классификатор.ПометкаУдаления", ВидСравненияКомпоновкиДанных.Равно, Ложь);
...
Процедура ПолучитьДанныеПриОткрытииФормы()
	...
	Если Не Отказ Тогда
		...
		Если ПолучатьДанныеПриОткрытииФормы Тогда
			ПодключитьОбработчикОжидания("ПолучитьДанныеКлассификатора", 0.1, Истина);
		КонецЕсли;
	ИначеЕсли ИсточникСведенийМЗ И ПредложитьЗаполнитьПараметрыЗагрузкиДанныхИзСервисаМинздраваРФ Тогда
		ПредложитьЗаполнитьПараметрыЗагрузкиДанныхИзСервисаМинздраваРФ();
	ИначеЕсли ИсточникСведенийФР И ПредложитьЗаполнитьПараметрыЗагрузкиДанныхИзСервисаФедеральныхРегистров Тогда
...
&НаСервере
Процедура УстановитьТипКлассификатора()
	
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() Тогда
		ТипКлассификатора = ОбменССервисомМинздраваРФ.ТипКлассификатора_Отделения();
	ИначеЕсли ИсточникСведений = ИсточникСведений_ФедеральныеРегистры() Тогда
		ТипКлассификатора = ОбменССервисомФедеральныхРегистров.ТипКлассификатора_Отделения();
	Иначе
		ВызватьИсключение(НСтр("ru = 'Неизвестный источник сведений. Обратитесь к администратору.'"));
	КонецЕсли;
...
&НаСервере
Функция ИсточникСведений_МинЗдрав()
...
&НаСервере
Функция ИсточникСведений_ФедеральныеРегистры()
	
	Возврат "ФедеральныеРегистры";
...
&НаСервере
Процедура УстановитьДоступныеВерсииКлассификатора()
	
	ИсточникСведенийМЗ = (ИсточникСведений = ИсточникСведений_МинЗдрав());
	
	Если ИсточникСведенийМЗ Тогда
		Если Не ЗначениеЗаполнено(Элементы.Версия.СписокВыбора) Тогда
			...
			Для Каждого ЭлементСписка Из СписокДоступныхВерсий Цикл
				Элементы.Версия.СписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
			КонецЦикла;
			
		КонецЕсли;
		
		Если Не УстанавливатьАктуальнуюВерсию И Элементы.Версия.СписокВыбора.НайтиПоЗначению(Версия) = Неопределено Тогда
			УстанавливатьАктуальнуюВерсию = Истина;
		КонецЕсли;
		
		Если УстанавливатьАктуальнуюВерсию Тогда
			Версия = Элементы.Версия.СписокВыбора[0].Значение;
			ВерсияДоИзменения = Версия;
		КонецЕсли;
	КонецЕсли;

	Элементы.Версия.Видимость = ИсточникСведенийМЗ;
