...
Функция ПолучитьДанныеКлассификатораВФоне()
	...
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() Тогда
		ФильтрыПолей = ОбменССервисомМинздраваРФ.ФильтрыПолейЗагрузкиДанныхКлассификатора_СтруктурныеПодразделения();
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если ТекущиеДанные.ЗагрузкаДоступна Тогда
			Если ИсточникСведенийМЗ Тогда
				ЭлементДанных = ОбменССервисомМинздраваРФ.СтруктураДанныхКлассификатора_СтруктурныеПодразделения();
			ИначеЕсли ИсточникСведенийФР Тогда
				ЭлементДанных = ОбменССервисомФедеральныхРегистров.СтруктураДанныхКлассификатора_СтруктурныеПодразделения();
			Иначе
				ВызватьИсключение(НСтр("ru = 'Неизвестный источник сведений. Обратитесь к администратору.'"));
			КонецЕсли;
...
Функция ОбработатьКэшированныеДанныеКлассификатора()
	...
	ЗагруженныеДанныеВерсии = ЗагруженныеДанныеОрганизации.Получить(ВерсияКэша);
...
Процедура ОчиститьКэшированныеДанные()
	...
	ЗагруженныеДанныеОрганизации.Вставить(ВерсияКэша, ЗагруженныеДанныеВерсии);
...
Процедура УстановитьУсловноеОформление()
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИ,
		"Классификатор.ДатаЛиквидации", ВидСравненияКомпоновкиДанных.Заполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИ,
...
Процедура УстановитьТипКлассификатора()
	...
	Если ИсточникСведений = ИсточникСведений_МинЗдрав() Тогда
		ТипКлассификатора = ОбменССервисомМинздраваРФ.ТипКлассификатора_СтруктурныеПодразделения();
	ИначеЕсли ИсточникСведений = ИсточникСведений_ФедеральныеРегистры() Тогда
		ТипКлассификатора = ОбменССервисомФедеральныхРегистров.ТипКлассификатора_СтруктурныеПодразделения();
	Иначе
		ВызватьИсключение(НСтр("ru = 'Неизвестный источник сведений. Обратитесь к администратору.'"));
	КонецЕсли;
