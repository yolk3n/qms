...
Процедура ЗаполнитьКоличествоПоУчету()
	...
	|	ЕСТЬNULL(ТоварыВОтделении.КоличествоОстаток, 0)   КАК КоличествоПоДаннымУчета,
	|	ВЫБОР 
	|		КОГДА ТаблицаНоменклатуры.Коэффициент = 0
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ТоварыВОтделении.КоличествоОстаток, 0) / ТаблицаНоменклатуры.Коэффициент
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладахВОтделениях.Остатки(
	|				&ДатаОстатков,
	|				(Номенклатура, СерияНоменклатуры, Партия, ИсточникФинансирования, Организация, Склад, МестоХранения) В
	|					(ВЫБРАТЬ
	...
	|						&МестоХранения
	|					ИЗ
	|						ТаблицаНоменклатуры КАК Таблица)) КАК ТоварыВОтделении
	|		ПО ТаблицаНоменклатуры.Номенклатура = ТоварыВОтделении.Номенклатура
	|			И ТаблицаНоменклатуры.СерияНоменклатуры = ТоварыВОтделении.СерияНоменклатуры
	|			И ТаблицаНоменклатуры.Партия = ТоварыВОтделении.Партия
	|			И ТаблицаНоменклатуры.ИсточникФинансирования = ТоварыВОтделении.ИсточникФинансирования
	|
	|УПОРЯДОЧИТЬ ПО
