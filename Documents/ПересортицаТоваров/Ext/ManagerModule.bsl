...
Функция ПолучитьТекстЗапросаДанныеДляПечати()
	...
	|	Документ.ИнвентаризацияТоваровНаСкладе             КАК Основание,
	|	Документ.Товары.(
...
Процедура ЗаполнитьНоменклатуруОприходованияОтложенноДанныеДляОбновления(Параметры) Экспорт
...
Процедура ЗаполнитьНоменклатуруОприходованияОтложенно(Параметры) Экспорт
	...
	|	Товары.Номенклатура      КАК НоменклатураОприходование,
	|	Товары.ЕдиницаИзмерения  КАК ЕдиницаИзмеренияОприходование,
	|	Товары.Коэффициент       КАК КоэффициентОприходование,
	|	Товары.Количество        КАК КоличествоОприходование
	|ИЗ
	|	Документ.ПересортицаТоваров.Товары КАК Товары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект = Неопределено Тогда
				ОтменитьТранзакцию();
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ВыборкаДокументы.Ссылка);
				Продолжить;
			КонецЕсли;
			
			ДокументОбъект.ПриходоватьТоварыПоСебестоимостиСписания = Истина;
			
			Выборка = ВыборкаДокументы.Выбрать();
			Пока Выборка.Следующий() Цикл
				НайденныеСтроки = ДокументОбъект.Товары.НайтиСтроки(Новый Структура("НомерСтроки", Выборка.НомерСтроки));
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать пересортицу товаров: %Ссылка% по причине: %Причина%'");
