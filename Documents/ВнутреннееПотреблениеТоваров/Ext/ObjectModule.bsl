...
Процедура ЗаполнитьДокументНаОснованииПоступленияТоваров(ДокументОснование)
	...
	Пока Выборка.Следующий() Цикл
		...
		СтатьяРасходов = ОбщегоНазначенияВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаТовары.Номенклатура, "had_СтатьяРасходов");
		Если ЗначениеЗаполнено(СтатьяРасходов) Тогда
			СтрокаТовары.СтатьяРасходов = СтатьяРасходов;
		КонецЕсли; 
		// } Сурина
