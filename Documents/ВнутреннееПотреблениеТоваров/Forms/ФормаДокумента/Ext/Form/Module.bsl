...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "ИзменениеСостоянияМДЛП"
			Или ИмяСобытия = "Запись_УведомлениеОВыбытииМДЛП"
			Или ИмяСобытия = "Запись_УведомлениеОПередачеНаУничтожениеМДЛП"
			Или ИмяСобытия = "Запись_УведомлениеОФактеУничтоженияМДЛП")
	...
	// ИнтеграцияРЭМД
	Если ИмяСобытия = "ИзменениеСостоянияСЭМД"
	   И Параметр.Ссылка = Объект.Ссылка Тогда
		СформироватьТекстСЭМД();
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ПодборТоваровКлиент.ОбработатьПодборТоваровПоОтбору(ЭтотОбъект, ИсточникВыбора) Тогда
		ОбработатьПодбор(ВыбранноеЗначение.АдресТоваровВХранилище, КэшированныеЗначения);
	ИначеЕсли ИнтеграцияРЭМДБольничнаяАптекаКлиент.ОбработатьПолучениеДанныхДокументовИзРЭМД(ВыбранноеЗначение, ИсточникВыбора) Тогда
		// ИнтеграцияРЭМД
		ОбработатьПолучениеДанныхДокументовИзРЭМД(ВыбранноеЗначение.АдресТоваровВХранилище, КэшированныеЗначения);
		// Конец ИнтеграцияРЭМД
	Иначе
		// БуферОбменаТоварами
...
&НаКлиенте
Процедура ПодобратьТоварыПоСЭМД(Команда)
	...
	ПараметрыФормы.Вставить("МестоХраненияОстатка"    , "Склад");
	ПараметрыФормы.Вставить("Склад"                   , Объект.Склад);
	ПараметрыФормы.Вставить("ИсточникФинансирования"  , Объект.ИсточникФинансирования);
	ПараметрыФормы.Вставить("РежимПодбораБезСуммовыхПараметров", Истина);
	
	ТипыНоменклатуры = ПодборТоваровКлиентСервер.ПолучитьОтборПоТипуНоменклатурыИзПараметровВыбора(Элементы.ТоварыНоменклатура.ПараметрыВыбора);
	ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", ТипыНоменклатуры);
	
	// Фиксированные настройки РЭМД. Считаем, что для этого документа входящи СЭМД может быть только ЛьготныйРецепт112.
	ТипВходящегоСЭМД = ИнтеграцияРЭМДБольничнаяАптекаВызовСервера.ВидыСЭМД().ЛьготныйРецепт.ТипыСЭМД.ЛьготныйРецепт112.ТипСЭМД;
	ПараметрыФормы.Вставить("ТипСЭМД", ТипВходящегоСЭМД);
	
	ИнтеграцияРЭМДБольничнаяАптекаКлиент.ОткрытьПолучениеДанныхДокументовИзРЭМД(ЭтотОбъект, ПараметрыФормы);
	// Конец ИнтеграцияРЭМД
	
КонецПроцедуры

&НаКлиенте
Процедура ТекстСЭМДОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	// ИнтеграцияРЭМД
	ИнтеграцияРЭМДБольничнаяАптекаКлиент.ТекстСЭМДОбработкаНавигационнойСсылки(ЭтотОбъект, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
	// Конец ИнтеграцияРЭМД
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	// Профрешение Сурина 12.01.24 {  
	СтатьяРасходов = ОбщегоНазначенияВызовСервера.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Номенклатура, "had_СтатьяРасходов");
	Если ЗначениеЗаполнено(СтатьяРасходов) Тогда
		ТекущаяСтрока.СтатьяРасходов = СтатьяРасходов;
	КонецЕсли;                                                                          	
	// } Сурина
...
Процедура СформироватьТекстУведомленияМДЛП()
	...
	СовместноВводимыеУведомления = Новый Соответствие;
	СовместноВводимыеУведомления.Вставить(Метаданные.Документы.УведомлениеОПередачеНаУничтожениеМДЛП, "1");
	СовместноВводимыеУведомления.Вставить(Метаданные.Документы.УведомлениеОФактеУничтоженияМДЛП, "1");
	ИнтеграцияМДЛП.СформироватьТекстУведомленияМДЛП(ЭтотОбъект, СовместноВводимыеУведомления);
...
&НаСервере
Процедура СформироватьТекстСЭМД()
	
	ИнтеграцияРЭМДБольничнаяАптека.СформироватьТекстГиперссылкиСЭМД(ЭтотОбъект);
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	СписокЭлементов.Добавить(Элементы.ТоварыПодобратьТоварыПоСЭМД.Имя);
...
Процедура ОбработатьПодбор(Знач АдресТоваровВХранилище, КэшированныеЗначения)
	...
	Для Каждого Товар Из СписокТоваров Цикл
		...
		// Профрешение Сурина 12.01.24 {
		СтатьяРасходов = ОбщегоНазначенияВызовСервера.ЗначениеРеквизитаОбъекта(НоваяСтрока.Номенклатура, "had_СтатьяРасходов");
		Если ЗначениеЗаполнено(СтатьяРасходов) Тогда
...
&НаСервере
Процедура ОбработатьПолучениеДанныхДокументовИзРЭМД(Знач АдресТоваровВХранилище, КэшированныеЗначения)
...
Процедура ВставитьТоварыИзБуфераОбменаСервер(Знач ВыбранныеТовары = Неопределено)
	...
	Отбор = ПодборТоваровКлиентСервер.ПолучитьОтборНоменклатурыПоПараметрамВыбора(Элементы.ТоварыНоменклатура);
	
	ДанныеВставлены = ОбработкаТабличнойЧастиСервер.ВставитьТоварыИзБуфераОбмена(ВыбранныеТовары, ТабличнаяЧасть, СтруктураДействий, Отбор);
