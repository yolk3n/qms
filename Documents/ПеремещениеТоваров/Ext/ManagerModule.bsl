...
Функция ПараметрыУчетаНоменклатуры(Объект) Экспорт
	...
	ПараметрыУчета.Отгружено = Объект.Статус = Перечисления.СтатусыПеремещенийТоваров.Отгружено Или Объект.Статус = Перечисления.СтатусыПеремещенийТоваров.Принято;
...
Функция ПодготовитьОсновныеДанныеДляПроведения(ДополнительныеСвойства)
	...
	ЗапрашиваемыеДанные.Вставить("ОрганизацияПолучатель", "ВЫБОР КОГДА ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами) ТОГДА ОрганизацияПолучатель ИНАЧЕ Организация КОНЕЦ");
	...
	ЗапрашиваемыеДанные.Вставить("ТипСкладаОтправитель", "СкладОтправитель.ТипСклада");
	ЗапрашиваемыеДанные.Вставить("СкладПолучатель");
	ЗапрашиваемыеДанные.Вставить("ТипСкладаПолучатель", "СкладПолучатель.ТипСклада");
	ЗапрашиваемыеДанные.Вставить("ЗаказНаПеремещение");
	...
	ОсновныеДанныеДокумента.Вставить(
		"РазделУчетаОтправитель",
		?(ОсновныеДанныеДокумента.ТипСкладаОтправитель = Перечисления.ТипыСкладов.Отделение,
			Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыВОтделениях,
			Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах));
	
	ОсновныеДанныеДокумента.Вставить(
		"РазделУчетаПолучатель",
		?(ОсновныеДанныеДокумента.ТипСкладаПолучатель = Перечисления.ТипыСкладов.Отделение,
			Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыВОтделениях,
			Перечисления.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах));
...
Функция ТекстЗапросаТоварыНаСкладах()
	...
	|
	|ГДЕ
	|	&Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыПеремещенийТоваров.Принято))
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	НомерСтроки
...
Функция ОписаниеКлючевыхПолей(Версия = Неопределено) Экспорт
	...
	ЭлектроннаяПодписьБольничнаяАптека.ДобавитьОписаниеРеквизита(ОписаниеКлючевыхПолей, ПоляШапки.ХозяйственнаяОперация.Имя);
