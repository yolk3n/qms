...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Пользователи.РолиДоступны("РазмещениеПоМестамХраненияВОтделениях") Тогда
		Элементы.Операция.СписокВыбора.Добавить(Перечисления.ОперацииРазмещенияПоМестамХраненияВОтделении.РазмещениеПоМестамХранения);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ХозяйственнаяОперация) И Элементы.Операция.СписокВыбора.НайтиПоЗначению(Объект.ХозяйственнаяОперация) = Неопределено Тогда
		Элементы.Операция.СписокВыбора.Добавить(Объект.ХозяйственнаяОперация);
		Элементы.Операция.ТолькоПросмотр = Истина;
	КонецЕсли;
...
Процедура КомандаПодбор(Команда)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ОперацииРазмещенияПоМестамХраненияВОтделении.ПередачаНаПост") Тогда
		Отбор.Вставить("МестоХранения", ПредопределенноеЗначение("Справочник.МестаХранения.ПустаяСсылка"));
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ОперацииРазмещенияПоМестамХраненияВОтделении.ВозвратСПоста") Тогда
...
&НаКлиенте
Процедура ЗаполнитьМестоХраненияОтправитель(Команда)
	
	ЗаполнитьРеквизитВВыделенныхСтроках(
		"МестоХранения",
		НСтр("ru='Место хранения (отправитель)'"),
...
&НаКлиенте
Процедура ЗаполнитьМестоХраненияПолучатель(Команда)
	
	ЗаполнитьРеквизитВВыделенныхСтроках(
		"МестоХраненияНовое",
		НСтр("ru='Место хранения (получатель)'"),
		"Справочник.МестаХранения.ФормаВыбора",
		Новый Структура("Отбор", Новый Структура("Владелец", Объект.Склад)));
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	Документы.РазмещениеТоваровПоМестамХраненияВОтделении.ЗаполнитьИменаРеквизитовПоТипуОперации(
		Объект.ХозяйственнаяОперация,
		ВсеЭлементы,
		ВидимыеЭлементы);
	
	ВсеЭлементы.Добавить(Элементы.ТоварыЗаполнитьМестоХраненияОтправитель.Имя);
	ВсеЭлементы.Добавить(Элементы.ТоварыЗаполнитьМестоХраненияПолучатель.Имя);
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ОперацииРазмещенияПоМестамХраненияВОтделении.РазмещениеПоМестамХранения Тогда
		ВидимыеЭлементы.Добавить(Элементы.ТоварыЗаполнитьМестоХраненияОтправитель.Имя);
		ВидимыеЭлементы.Добавить(Элементы.ТоварыЗаполнитьМестоХраненияПолучатель.Имя);
	КонецЕсли;
...
Процедура ОбработатьИзменениеОтделения()
	...
	Если ТекущийСклад <> Объект.Склад И ЗначениеЗаполнено(Объект.Склад) Тогда
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьМестаХранения", Новый Структура("Склад", Объект.Склад)) Тогда
...
Процедура ОбработатьИзменениеСклада()
	...
	ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьКолонкиТабличнойЧасти(Объект.Товары, Неопределено, "МестоХранения, МестоХраненияНовое");
